...
class LazySupervisedDataset(Dataset):
    ...
    def multi_instance_multi_modal_get_item(self, data_item):
        image_cnt = 0
        for idx, conv in enumerate(data_item['conversations']):
            # conv['value'] = conv['value'].replace('<image>\n', '').replace('\n<image>', '').replace('<image>', '')
            image_cnt += conv['value'].count('<image>')
            if conv['from'] == 'human':
                conv['value'] = self.base_prompt + conv['value']
        ...
        for image in data_item['image']:
            ...
            transform = build_transform(is_train=self.is_train, input_size=self.image_size,
                                        pad2square=self.pad2square, normalize_type=self.normalize_type)
            if self.dynamic_image_size:
                images.append(dynamic_preprocess(image, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch,
                                            image_size=self.image_size, use_thumbnail=self.use_thumbnail))
            else:
                images.append(image)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        while True:
            ...
            if 'image' in data_item and isinstance(data_item['image'], list):
                ret = self.multi_instance_multi_modal_get_item(data_item)
...
def main():
    ...
    if model_args.model_name_or_path is not None:
        ...
        model = InternVLChatModelMI.from_pretrained(
        ...
        model = InternVLChatModelMI(internvl_chat_config, vision_model, llm)
