...
def preprocess_mpt(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if sentence['from'] == 'gpt':
                ...
                sent = sentence['value'].translate(translator)
                for s in sent.split(' '):
                    if s in extracted_words:
                        key_embeddings.add(s)
                    elif s.isdigit():
                        for ss in s:
                            key_embeddings.add(ss)
            assert role == conv.roles[j % 2], f'{i}'
            if text_only:
                sentence['value'] = sentence['value'].replace('<image>', '').replace('<query>', '')
            conv.append_message(role, sentence['value'])
    ...
    targets = input_ids.clone()
    key_embeddings = [tokenizer(
        key_embedding,
        return_tensors='pt',
        padding=False if group_by_length else 'max_length',
        max_length=tokenizer.model_max_length,
        truncation=True,
    ).input_ids[0] for key_embedding in key_embeddings]
    ...
    loss_reweight = torch.ones(targets.shape)
    for loss_re, target in zip(loss_reweight, targets):
        for i in range(len(target)):
            if target[i] == -100:
                loss_re[i] = 0
            else:
                for key_embedding in key_embeddings:
                    if torch.equal(target[i:i+len(key_embedding)], key_embedding):
                        loss_re[i:i+len(key_embedding)] = 2
