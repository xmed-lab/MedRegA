...
class MLVLFuseModule(nn.Module):
    def __init__(self, input_dims=1024, embed_dims=1024, num_levels=3, num_fuse=2):
    ...
    def _single_shuffle(self, inputs, conv_module, k):
        if not isinstance(conv_module, (nn.ModuleList, list)):
            conv_module = [conv_module]

        for single_conv_m in conv_module:
            fused_inputs = []
            for l, fuse_lvl_tuple in enumerate(self.fuse_lvl_list):
    ...
    def forward(self, inputs):
        ...
        # new_inputs = []
        # for l, (feat, single_feat_size) in enumerate(zip(inputs, feat_size)):
        ...
        for k, conv_m in enumerate(self.fuse_convs):
            inputs = self._single_shuffle(inputs, [conv_m], k)
        return inputs


class MLVLROIQueryModule(nn.Module):
    def __init__(self, embed_dims=3200, out_dims=4096,
                 num_levels=3):
    ...
    def forward(self, mlvl_feats, bboxes):
        if mlvl_feats[0].dim() == 3:
            h = w = int(math.sqrt(mlvl_feats[0].shape[1]))  # h = w = 32
            # assert h == 24
            # assert w == 24
            b, c = mlvl_feats[0].shape[0], mlvl_feats[0].shape[-1]
            mlvl_feats = [item.reshape(b, h, w, c).permute(0, 3, 1, 2) for item in mlvl_feats]  # [1, 3200, 32, 32]
...
class MlvlRoIExtractor(BaseRoIExtractor):
    def __init__(self,
        ...
        self.strides = [14 / 8, 14 / 4, 14 / 2, 14]
        
        # self.roi_layers = nn.ModuleList([MultiScaleRoIAlign(featmap_names=['0'], output_size=14, sampling_ratio=2)])
    ...
    def forward(self, feats, rois, roi_scale_factor=None):
        """Forward function."""
        num_imgs = len(rois)
        batch_rois = torch.tensor(rois, dtype=feats[0].dtype).to(feats[0].device)
        ...
        new_rois = []
        for img_id, single_img_roi in enumerate(batch_rois):
            # rescale to original img scale
            # single_img_roi = single_img_roi * 224
            roi_img_id = single_img_roi.new_ones(1) * img_id
            single_img_roi = torch.cat([roi_img_id, single_img_roi], dim=0)
            new_rois.append(single_img_roi)
        rois = torch.stack(new_rois)
        roi_feats = feats[0].new_zeros(self.fuse_level,
                                       rois.size(0), self.out_channels, 14)
        ...
        for i in range(num_levels):
            ...
            if len(rois) > 0:
                ...
                pdb.set_trace()
                roi_feats_t = self.roi_layers[i](feats[i].to(torch.float32), rois_.to(torch.float32))  ## 感觉可能是维度弄得不对
                roi_feats[i] = roi_feats_t.to(ori_dtype)
                # roi_feats_t = roi_align(feats[i].to(torch.float32), rois_.to(torch.float32), output_size=(14, 14), sampling_ratio=2, spatial_scale=(1 / self.strides[i]))
                # roi_feats_t = roi_pool(feats[i].to(torch.float32), rois_.to(torch.float32), output_size=(14, 14), spatial_scale=(1 / self.strides[i]))
                # roi_feats[i] = roi_feats_t.to(ori_dtype)
            else:
                roi_feats += sum(
                    x.view(-1)[0]
